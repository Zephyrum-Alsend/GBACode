...
int main(void)
{
	...
	for (int i = 0; i < 4 * (sizeof(tile_4bpp) / 2); ++i)  {
		if(i%4==0) // Sets how tall each checker is, where p = x/2
			checkered = !checkered;
		if(checkered)
			paddle_tile_mem[i] = 0x2211; // 0b_0002_0002_0001_0001
		else
			paddle_tile_mem[i] = 0x1122;
	}
	...
	// Do not go above 0x1F
	object_palette_mem[1] = RGB15(0x1F, 0x1F, 0x0F); // Yellow
	object_palette_mem[2] = RGB15(0x00, 0x0F, 0x00); // Green
	...
	const int ball_width = 8,
	          ball_height = 8;
	int player_normal_velocity = 2,
		player_sprint_velocity = 4,
	...
	while (1) {
		...
		// Hold down the A button to double movement speed.
		if (key_states & KEY_A)
			player_velocity = player_sprint_velocity;
		else
			player_velocity = player_normal_velocity;
		...
		if ((ball_x >= player_x &&
		    ...
		    (ball_y + ball_height >= player_y &&    // Without "+ ball_height" the ball clips through the top of the paddle.
